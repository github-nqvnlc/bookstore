...
let create_payment_url = (req, res) => {
    ...
    let orderId = ("0" + date.getHours()).slice(-2) + ("0" + date.getMinutes()).slice(-2) + ("0" + date.getSeconds()).slice(-2)
...
let get_payment_return = (req, res) => {
    ...
    if (secureHash === signed) {
        res.redirect(`http://localhost:3000/payment-return/${vnp_Params['vnp_ResponseCode']}/${vnp_Params['vnp_OrderInfo']}`)
    } else {
